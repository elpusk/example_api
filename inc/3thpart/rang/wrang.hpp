...
namespace rang_implementation {
    ...
    inline bool isTerminal(const std::wstreambuf *osbuf) noexcept
    {
        ...
        if (osbuf == wcout.rdbuf()) {
            ...
            return cout_term;
        } else if (osbuf == wcerr.rdbuf() || osbuf == wclog.rdbuf()) {
    ...
    inline enableStd<T> setColor(std::wostream &os, T const value)
    {
        return os << L"\033[" << static_cast<int>(value) << L"m";
    }
#endif
}  // namespace rang_implementation

template <typename T>
inline rang_implementation::enableStd<T> operator<<(std::wostream &os,
                                                    const T value)
{
